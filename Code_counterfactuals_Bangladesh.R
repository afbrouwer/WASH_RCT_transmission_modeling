...
prevalence_by_arm_new_coverage = function(par, new_omega, dataset){
  ...
  for (j in 0:2){
    for (i in 1:length(clusters)){
      ...
      prev= 0.06 #baseline prevalence 
      x0 = c(rep(prev,16)*(rho_vec*omega + intervention_adherence_vec*(new_omega- omega)+ baseline_adherence_vec*(1-new_omega)),
             rep(1-prev, 16)*(rho_vec*omega + intervention_adherence_vec*(new_omega- omega) + baseline_adherence_vec*(1-new_omega)), prev, prev ,prev)
...
prevalence_by_arm_no_conditions = function(par, dataset){
  ...
  for (j in 0:2){
    for (i in 1:length(clusters)){
      ...
      prev= 0.06 #baseline prevalence 
      x0 = c(rep(prev,16)*(rho_vec*omega + new_baseline_adherence_vec*(1-omega)),
             rep(1-prev, 16)*(rho_vec*omega + new_baseline_adherence_vec*(1-omega)), prev, prev ,prev)
...
counterfactual_simulation = function(i, simul){
  ...
  if (simul == "no_arm_time_var"){ 
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1
    prev_mat = prevalence_by_arm(par=par, data=data)}
  #counterfactual - coverage = 15%
  if (simul == "coverage15"){ 
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par,new_omega = 0.15, data=data)}
  #counterfactual - coverage = 10%
  if (simul == "coverage10"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.1, data=data)}
  #counterfactual - coverage = 20%
  if (simul == "coverage20"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.2, data=data)}
  #counterfactual - coverage = 30%
  if (simul == "coverage30"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.3, data=data)}
  #counterfactual - coverage = 40%
  if (simul == "coverage40"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.4, data=data)}
  #counterfactual - coverage = 50%
  if (simul == "coverage50"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.5, data=data)}
  #counterfactual - coverage = 60%
  if (simul == "coverage60"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.6, data=data)}
  #counterfactual - coverage = 70%
  if (simul == "coverage70"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.7, data=data)}
  #counterfactual - coverage = 80%
  if (simul == "coverage80"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.8, data=data)}
  #counterfactual - coverage = 90%
  if (simul == "coverage90"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 0.9, data=data)}
  #counterfactual - coverage = 100%
  if (simul == "coverage100"){
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_new_coverage(par=par, new_omega = 1, data=data)}
  # counterfactual - no conditions 
  if (simul == "no_conditions"){ 
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_no_conditions(par=par, data=data)}
  # counterfactual - full adherence 
  if (simul == "full_adherence"){ 
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    prev_mat = prevalence_by_arm_full_adherence(par=par, data=data)}
  ...
  if (simul == "50_less_other"){ 
    ...
    prev_mat = prevalence_by_arm(c(par[1],(R0W/par[1]), R0H/(par[1]-R0W), par[4:16]), data=data)}
  ...
  if (simul == "100_less_other"){ 
    ...
    prev_mat = prevalence_by_arm(c(par[1],(R0W/par[1]), 1, par[4:16]), data=data)}
  ...
  if (simul == "25efficacy_S"){ 
    par=unlist(sample_and_NLL[i,1:16])
    # par[9:16] = 1 
    par[7]= min(1, 1.25*par[7])
    prev_mat = prevalence_by_arm(par, data=data)}
  # counterfactual - 25% more efficacy of S
  if (simul == "50efficacy_S"){ 
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    par[7]= min(1, 1.5*par[7])
    prev_mat = prevalence_by_arm(par, data=data)}
  if (simul == "100efficacy_S"){ 
    ...
    par[7]= min(1, 2*par[7])
    prev_mat = prevalence_by_arm(par, data=data)}
  if (simul == "2prev"){ #nprev is valid for x between 1 and n/(n-1)
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    par[1]= par[1]/(par[1]+2-2*par[1])
    prev_mat = prevalence_by_arm(par, data=data)}
  if (simul == "3prev"){ 
    par=unlist(sample_and_NLL[i,1:16])
    par[9:16] = 1 
    par[1]= par[1]/(par[1]+3-3*par[1])
    prev_mat = prevalence_by_arm(par, data=data)}
...
# index_prevalences = sapply(index[1],FUN=counterfactual_simulation, simul="full_adherence")
...
# 
# tic()
# index_prevalences = parSapply(cluster,index,FUN=counterfactual_simulation, simul="coverage15")
# toc()
# index_prevalences = cbind(index, t(index_prevalences))
# full_prevalences = index_prevalences[match(resample,index),-1]
# saveRDS(full_prevalences, "Bprevalences_coverage15.RDS")
# print("coverage15")
# 
# tic()
# index_prevalences = parSapply(cluster,index,FUN=counterfactual_simulation, simul="coverage10")
# toc()
# index_prevalences = cbind(index, t(index_prevalences))
# full_prevalences = index_prevalences[match(resample,index),-1]
# saveRDS(full_prevalences, "Bprevalences_coverage10.RDS")
# print("coverage10")
# 
# tic()
